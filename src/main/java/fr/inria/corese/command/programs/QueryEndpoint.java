...
@Command(
        name = "query-endpoint",
        description = "Execute a SPARQL query on a remote endpoint.",
...
public class QueryEndpoint extends AbstractCommand {
    ...
    @Option(
            names = {"-q", "--query"},
            description =
                    "Specifies the SPARQL query to execute. This can be provided as a URL or a file"
                            + " path.",
            required = false)
    ...
    @Option(
            names = {"-e", "--endpoint"},
            description = "Specifies the SPARQL endpoint URL.",
            required = true)
    private String endpointUrl;

    @Option(
            names = {"-H", "--header"},
            description = "Adds an HTTP header to the request. Multiple headers can be specified.",
            arity = "0..")
    private List<String> headers;

    @Option(
            names = {"-a", "-of", "--accept"},
            description = "Specifies the Accept header value for the HTTP request.",
            arity = "0..")
    private List<String> accepts;

    @Option(
            names = {"-m", "--request-method"},
            description =
    ...
    @Option(
            ...
            description = "Specifies the default graph URI. Multiple URIs can be specified.",
            arity = "0..")
    private List<String> defaultGraph;

    @Option(
            names = {"-n", "--named-graph"},
            description = "Specifies the named graph URI. Multiple URIs can be specified.",
            arity = "0..")
    private List<String> namedGraph;

    @Option(
            names = {"-i", "--ignore-query-validation"},
            description =
                    "Ignores query validation if set to true. Default value: ${DEFAULT-VALUE}.",
            required = false,
            defaultValue = "false")
    ...
    private static final String DEFAULT_ACCEPT_HEADER = "text/csv";
    ...
    public Integer call() {
        ...
        try {
            ...
            if ((this.accepts == null || this.accepts.isEmpty())
                ...
                this.accepts = List.of(QueryEndpoint.DEFAULT_ACCEPT_HEADER);
    ...
    private boolean containsAcceptHeader(List<String> headers) {
        return headers != null
                && headers.stream()
                        .anyMatch(header -> header.trim().toLowerCase().startsWith("accept:"));
    }
    ...
    public String sendRequest() throws Exception {
        ...
        return client.sendRequest(
                this.query, this.defaultGraph, this.namedGraph, this.ignoreQueryValidation);
    }
            ...
            for (String header : this.headers) {
                ...
                if (headerParts.length == 2) {
                    ...
                    throw new IllegalArgumentException(
                            "Invalid header format (expected key: value): " + header);
                }
            }
        }
    }
        ...
        if (this.output != null) {
            ...
            try {
                ...
                throw new UncheckedIOException(
                        "Error while writing result to file: " + e.getMessage(), e);
            }
        } else {
